...
CANDIDATES = ROOT / "data" / "annotations" / "live_prompt_calibration_candidates.yaml"


def test_final_bundle_has_35_formal_medoids_plus_one_reviewed_live_candidate() -> None:
    ...
    assert payload["base_prototype_count"] == 35
    assert payload["live_calibration_candidate_count"] == 1
    assert payload["prototype_count"] == 36
    assert len(loaded.model.prototypes) == 36
...
def test_final_calibration_is_conservative_aggregation_of_seven_loso_folds() -> None:
    ...
    assert loaded.bundle["threshold_aggregation"]["live_candidates_used_for_threshold_calibration"] is False


def test_live_candidate_is_reviewed_calibration_evidence_not_ground_truth() -> None:
    payload = yaml.safe_load(CANDIDATES.read_text(encoding="utf-8"))
    assert payload["status"] == "live_calibration_candidate"
