...
RESULT_PENDING arms Press/Get and uses medium Prompt polling. PRESS is a three-layer contract: grid/timer geometry creates `PRESS_PANEL_CANDIDATE`, consecutive structural confirmation creates `PRESS_PANEL_PRESENT` and may confirm runtime PRESS, and variable-length temporal glyph consensus creates `PRESS_SEQUENCE_READY`. Glyph confidence never gates panel presence. Only sequence-ready evidence may propose one `PRESS_SEQUENCE` intent per panel episode; panel disappearance moves PRESS to RESULT_PENDING without inventing a sequence. GetPanel has priority over `IDLE_CAST` and confirms GET.
