...
| `src/detectors/hook_detector.py` | reuse_via_adapter | Offline detector returns detected/confidence/debug/fill evidence. Threshold and implementation remain untouched. |
| `src/detectors/press_detector.py` | reuse_via_adapter | Offline detector returns detected/confidence/sequence/debug. Adapter hides its dictionary and template internals. |
| `src/detectors/get_detector.py` | reuse_via_adapter | Strict title/grid/button detector is suitable specialized evidence; adapter catches exceptions. |
...
| `src/ml/prompt_*` / PromptClassifier v1 | reference_only | Model is frozen and may support analysis only. It is not a v2 observer and its threshold is not reused. |
